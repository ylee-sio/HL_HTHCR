      ...
      do
        ...
        while [ $test_probe != "pass" ]
          do
            ...
            if [ -d "$psp/$species_name/$species_initial/$accession" ]; then
              test_probe="pass"
              let "probe_num+=1"
              max_possible_probe_pairs=$(cat $psr/accessions_by_species/$species_name.$species_accession/.num_probe_pairs_log/max_list.csv | grep $accession | cut -d , -f 5)
              echo "The maximum number of probe pairs generatable for this accession is" $max_possible_probe_pairs
              read -p 'Enter number of pairs dedicated for this target: ' num_probe_pairs
              echo " "
              bash $pss/stock_shop_2.sh $accession $species_name $species_initial $num_probe_pairs
              echo " "
              ...
              test_probe="fail"
	      echo " "
              echo "Probes for $accession do not exist."
              echo "Please re-enter an existing NCBI protein accession ID."
